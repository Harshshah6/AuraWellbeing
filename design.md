...
The application supports a dark mode toggled via `[data-theme='dark']` on the `<html>` root element. Dark mode inverts surfaces, adjusts text contrast, and shifts accent colors to their on-dark variants. Theme preference is persisted to `localStorage`.

### Dark Mode Tokens

| Light Token | Light Value | Dark Value | Notes |
|---|---|---|---|
| `{colors.primary}` | #0066cc | #2997ff | Shifts to Sky Link Blue for visibility on dark surfaces |
| `{colors.primary-focus}` | #0071e3 | #64b5f6 | Lighter focus ring on dark backgrounds |
| `{colors.canvas}` | #ffffff | #1c1c1e | Primary surface — iOS-inspired dark gray, not pure black |
| `{colors.canvas-parchment}` | #f5f5f7 | #141416 | Near-black alternating surface |
| `{colors.surface-pearl}` | #fafafc | #2c2c2e | Secondary surface for buttons/controls |
| `{colors.ink}` | #1d1d1f | #f5f5f7 | Primary text inverts to off-white |
| `{colors.ink-muted-80}` | #333333 | #d1d1d6 | Secondary text |
| `{colors.ink-muted-48}` | #7a7a7a | #8e8e93 | Tertiary/disabled text |
| `{colors.divider-soft}` | rgba(0,0,0,0.04) | rgba(255,255,255,0.06) | Subtle dividers |
| `{colors.hairline}` | #e0e0e0 | #38383a | Card borders and separators |
| `{colors.success}` | #059669 | #30d158 | Brighter green for dark backgrounds |
| `{colors.warning}` | #d97706 | #ffd60a | Brighter yellow for dark backgrounds |
| `{colors.danger}` | #e11d48 | #ff453a | Brighter red for dark backgrounds |

### Dark Mode Principles

- **No pure black backgrounds.** Dark surfaces use iOS-inspired gray tones (#1c1c1e, #141416) to maintain visual depth.
- **Elevated cards remain distinguishable.** `{colors.canvas}` (#1c1c1e) on dark is one step lighter than `{colors.canvas-parchment}` (#141416), preserving the alternating tile rhythm.
- **Accent shifts to on-dark variants.** The primary blue shifts from #0066cc to #2997ff for readability.
- **Shadows deepen.** Product dial shadows increase opacity (0.22 → 0.5) to remain visible against dark surfaces.
- **Segmented pill shadows use stronger alpha** (0.3 instead of 0.06) to maintain the active-state elevation cue.
